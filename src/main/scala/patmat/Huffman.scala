...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Fork(left, right, _, weight) => weight + this.weight(left) + this.weight(right)
    case Leaf(_, weight) => weight
  }
  
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(left, right, chars, _) => chars ::: this.chars(right) ::: this.chars(left)
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] =
      chars.groupBy(tuple => tuple).map(tuple => (tuple._1, tuple._2.length)).toList
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
        ...
        case Fork(left: CodeTree, _, _, _) if bits.head == 0 => traverse(left, bits.tail)
